...
CzSEOWallet::CzSEOWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSEOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSEOSeed_deprecated()) {
                    LogPrintf("%s: Updated zSEO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSEOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSEOCount(nCountLastUsed);
    else if (!walletdb.ReadZSEOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSEOWallet::Lock()
{
    seedMaster = 0;
}

void CzSEOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSEOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSEO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSEOWallet::LoadMintPoolFromDB()
{
...
void CzSEOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSEOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSEOWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSEOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSEO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSEOWallet::SeedToZSEO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSEOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSEOWallet::GenerateDeterministicZSEO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSEOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSEOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
