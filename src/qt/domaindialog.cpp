...
DomainDialog::DomainDialog(QWidget* parent) : QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
...
void DomainDialog::setClientModel(ClientModel* clientModel)
{
...
void DomainDialog::setModel(WalletModel* model)
{
    this->model = model;

    if (model && model->getOptionsModel()) {
        for (int i = 0; i < ui->entries->count(); ++i) {
            DomainEntry* entry = qobject_cast<DomainEntry*>(ui->entries->itemAt(i)->widget());
            if (entry) {
...
void DomainDialog::on_sendButton_clicked()
{
    if (!model || !model->getOptionsModel())
        return;
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_SEO, true));
        if (!ctx.isValid()) {
...
void DomainDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
...
void DomainDialog::reject()
{
    clear();
}

void DomainDialog::accept()
{
    clear();
}

DomainEntry* DomainDialog::addEntry()
{
    DomainEntry* entry = new DomainEntry(this);
    entry->setModel(model);
    ui->entries->addWidget(entry);
    connect(entry, SIGNAL(removeEntry(DomainEntry*)), this, SLOT(removeEntry(DomainEntry*)));
...
void DomainDialog::updateTabsAndLabels()
{
    setupTabChain(0);
    coinControlUpdateLabels();
}

void DomainDialog::removeEntry(DomainEntry* entry)
{
    entry->hide();
...
QWidget* DomainDialog::setupTabChain(QWidget* prev)
{
    for (int i = 0; i < ui->entries->count(); ++i) {
        DomainEntry* entry = qobject_cast<DomainEntry*>(ui->entries->itemAt(i)->widget());
        if (entry) {
...
void DomainDialog::setAddress(const QString& address)
{
    DomainEntry* entry = 0;
    // Replace the first entry if it is still unused
    if (ui->entries->count() == 1) {
        DomainEntry* first = qobject_cast<DomainEntry*>(ui->entries->itemAt(0)->widget());
        if (first->isClear()) {
            entry = first;
        }
    }
...
void DomainDialog::pasteEntry(const SendCoinsRecipient& rv)
{
...
bool DomainDialog::handlePaymentRequest(const SendCoinsRecipient& rv)
{
...
void DomainDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, 
...
void DomainDialog::updateDisplayUnit()
{
...
void DomainDialog::updateSwiftTX()
{
    QSettings settings;
...
void DomainDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn& sendCoinsReturn, const QString& msgArg, bool fPrepare)
{
    ...
    // This comment is specific to DomainDialog usage of WalletModel::SendCoinsReturn.
...
void DomainDialog::minimizeFeeSection(bool fMinimize)
{
    ui->labelFeeMinimized->setVisible(fMinimize);
    ui->buttonChooseFee->setVisible(fMinimize);
    ui->buttonExplorer->setVisible(fMinimize);
...
void DomainDialog::on_buttonChooseFee_clicked()
{
    minimizeFeeSection(false);
}

void DomainDialog::on_buttonMinimizeFee_clicked()
{
    updateFeeMinimizedLabel();
    minimizeFeeSection(true);
}

void DomainDialog::setMinimumFee()
{
    ui->radioCustomPerKilobyte->setChecked(true);
    ui->customFee->setValue(CWallet::minTxFee.GetFeePerK());
}

void DomainDialog::updateFeeSectionControls()
{
...
void DomainDialog::updateGlobalFeeVariables()
{
...
void DomainDialog::updateFeeMinimizedLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
void DomainDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
...
void DomainDialog::updateSmartFeeLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
// UTXO splitter
void DomainDialog::splitBlockChecked(int state)
{
    if (model) {
...
//UTXO splitter
void DomainDialog::splitBlockLineEditChanged(const QString& text)
{
...
void DomainDialog::coinControlButtonClicked()
{
...
void DomainDialog::coinControlChangeChecked(int state)
{
...
void DomainDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid SEOCOIN address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
...
void DomainDialog::on_buttonExplorer_clicked()
{
	DomainExplorer* dialog = new DomainExplorer(this);
...
void DomainDialog::coinControlUpdateLabels()
{
