...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
	...
	QString domainRecord = "<data>" + rcp.offer + " " + rcp.domain + " visitors daily " + rcp.traffic + " " + rcp.contact + "</data>";

        customScript << OP_RETURN << ParseHex(domainRecord.toLatin1().toHex());
        vecSend.push_back(std::pair<CScript, CAmount>(customScript, (CAmount)0));
    }
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered seocoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SEO.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal seocoin:URI (seocoin:XyZ...?message=example)
            {
...
static void NotifyzSEOReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSEOReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSEOReset.connect(boost::bind(NotifyzSEOReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSEOReset.disconnect(boost::bind(NotifyzSEOReset, this));
