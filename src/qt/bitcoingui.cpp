                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("SEOCOIN Core") + " - ";
#ifdef ENABLE_WALLET
    ...
    if (enableWallet) {
        ...
        domainWindow = new DomainExplorer(this);
    ...
    connect(openDomainExplorerAction, SIGNAL(triggered()), domainWindow, SLOT(show()));

    // prevents an open debug window from becoming stuck/unusable on client shutdown
    connect(quitAction, SIGNAL(triggered()), explorerWindow, SLOT(hide()));
    connect(quitAction, SIGNAL(triggered()), domainWindow, SLOT(hide()));
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SEOCOIN address"));
    ...
    domainAction = new QAction(QIcon(":/icons/domain"), tr("&Domain"), this);
    domainAction->setStatusTip(tr("Add domain"));
    domainAction->setToolTip(domainAction->statusTip());
    domainAction->setCheckable(true);
#ifdef Q_OS_MAC
    domainAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
    domainAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
#endif
    tabGroup->addAction(domainAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/receiving_addresses"), tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and seocoin: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zSEO"));
    ...
    connect(domainAction, SIGNAL(triggered()), this, SLOT(gotoDomainPage()));

    connect(domainAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(domainAction, SIGNAL(triggered()), this, SLOT(gotoDomainPage()));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About SEOCOIN Core"), this);
    aboutAction->setStatusTip(tr("Show information about SEOCOIN Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for SEOCOIN"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SEOCOIN addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SEOCOIN addresses"));
    ...
    openAction->setStatusTip(tr("Open a SEOCOIN: URI or payment request"));

    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    openDomainExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Domain explorer"), this);
    openDomainExplorerAction->setStatusTip(tr("Domain explorer window"));


    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the SEOCOIN Core help message to get a list with possible SEOCOIN command-line options"));
...
void BitcoinGUI::createMenuBar()
{
    ...
    if (walletFrame) {
        ...
        tools->addAction(openDomainExplorerAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    domainAction->setEnabled(enabled);
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("SEOCOIN Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::createTrayIconMenu()
{
    ...
    trayIconMenu->addAction(domainAction);
    ...
    trayIconMenu->addAction(openDomainExplorerAction);
...
void BitcoinGUI::gotoDomainPage()
{
    domainAction->setChecked(true);
    if (walletFrame) walletFrame->gotoDomainPage();
}
...
void BitcoinGUI::gotoDomainExplorerPage()
{
    if (walletFrame) walletFrame->gotoDomainExplorerPage();
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SEOCOIN network", "", count));
}
...
{
    QString strTitle = tr("SEOCOIN Core"); // default title
