// Copyright (c) 2017-2018 The SEOCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "domainexplorer.h"
...
static CAmount getTxIn(const CTransaction& tx)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
        Sum += getPrevOutDomain(tx.vin[i].prevout).nValue;
    return Sum;
}
...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::SEO, nValue);
...
static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(), const std::string& Prepend = std::string(), int64_t* pSum = NULL)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        if (tx.IsCoinBase()) {
            ...
            CTxOut PrevOut = getPrevOutDomain(tx.vin[j].prevout);
...
const CBlockIndex* getexplorerBlockIndexDomain(int64_t height)
{
    std::string hex = getexplorerBlockHashDomain(height);
    uint256 hash = uint256S(hex);
    return mapBlockIndex[hash];
}

std::string getexplorerBlockHashDomain(int64_t Height)
{
...
std::string BlockToStringDomain(CBlockIndex* pBlock)
{
    if (!pBlock)
        return "";
...
//std::string TxToStringDomain(uint256 BlockHash, const CTransaction& tx)
std::string TxToStringDomain(const CTransaction& tx)

{
    ...
    if (tx.IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            CTxOut PrevOut = getPrevOutDomain(tx.vin[i].prevout);
            if (PrevOut.nValue < 0)
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        getNextInDomain(COutPoint(TxHash, i), HashNext, nNext);
...
std::string AddressToStringDomain(const CBitcoinAddress& Address)
{
    std::string TxLabels[] =
        {
            _("Date"),
...
DomainExplorer::DomainExplorer(QWidget* parent) : QMainWindow(parent),
                                                ui(new Ui::DomainExplorer),
                                                m_NeverShown(true),
                                                m_HistoryIndex(0)
{
    ui->setupUi(this);
...
void DomainExplorer::keyPressEvent(QKeyEvent* event)
{
...
void DomainExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        m_NeverShown = false;
	...
	if (boost::filesystem::exists(pathDomain))
        {
		std::ifstream t(pathDomain.string());
		std::stringstream buffer;
		buffer << "<span style='fontsize:12;color:#43464c'>" << t.rdbuf() << "</span>";
		...
		while (true) {
		     ...
		     file_content.replace(index, 1, "<br>");
		     index += 4;
		}
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (seocoin.conf).");
            QMessageBox::warning(this, "SEOCOIN Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
std::string DomainExplorer::SearchTransactionDomain(std::string search)
{

LogPrintf("SearchTransactionDomain\n");

    int tHeight = chainActive.Height();
    CBlockIndex* pindexSlow = NULL;
    std::string results = "<span style='font-size:14px;color:#873600'>Offer domain name visitors daily contact info price<br></span>";
	...
	LogPrintf("Search block %d\n", pindex);
		...
		std::string tr_list;
            ...
            BOOST_FOREACH (const CTransaction& tx, block.vtx)
            {
                ...
                std::string tr_text = TxToStringDomain(tx);

                if (tr_text.find("RETURN") != std::string::npos)
		{
			
//LogPrintf("TxToStringDomain(tx) %s\n", TxToStringDomain(tx));
				
			std::string token = tr_text.substr(tr_text.find("RETURN"), tr_text.length()-tr_text.find("RETURN"));
                        
                        std::string hex_value_2 = token.substr(token.find("0x") + 2, token.length()-token.find("0x"));
                        std::string hex_value_3 = hex_value_2.substr(hex_value_2.find("0x") + 2, hex_value_2.length()-hex_value_2.find("0x"));
                        std::string hex_value_4 = hex_value_3.substr(0, hex_value_3.find("<"));

LogPrintf("found %s\n", hex_value_4);

			QByteArray ar1  =  QString::fromStdString(hex_value_4).toLatin1();
			QByteArray ar2 = QByteArray::fromHex(ar1);
			QString outQString = QString::fromLatin1(ar2.data(), ar2.size());

//LogPrintf("string %s\n", outQString.toUtf8().constData());

			//tr_list.append(str2);
			if (outQString.contains(QString::fromStdString(search).toLatin1()))
			{
				results.append("<span style='font-size:14px;color:#873600'>").append(outQString.toUtf8().constData()).append("</span>");

LogPrintf("found %s\n", outQString.toLatin1().constData());
	...
	if(results.length() == 0) results.append("<span style='font-size:14px;color:#873600'>Nothing found</span><br>");

    return results;
}


bool DomainExplorer::switchTo(const QString& query)
{
    bool IsOk;
    int64_t AsInt = query.toInt(&IsOk);

	//diomain search
	std::string content = "";//SearchTransactionDomain(query.toLatin1().constData());
	std::string found;
	...
	boost::filesystem::path pathDomain = GetDataDir() / "seoindex.dat";
		...
		{
			if(s.find(query.toLatin1().constData()) != std::string::npos)			
			{			
				found.append("<span style='color: black;'>");
				found.append(s);
				found.append("</span><br>");
			}
		}
	...
	if(found.length() > 0) setContent(found);
	else setContent("<span style='fontsize:12;color:#43464c'>matches not found</span>");
	
	return true;

    // If query is integer, get hash from height
    if (IsOk && AsInt >= 0 && AsInt <= chainActive.Tip()->nHeight) {
        std::string hex = getexplorerBlockHashDomain(AsInt);
        uint256 hash = uint256S(hex);
    ...
    if (GetTransaction(hash, tx, hashBlock, true)) {
        ///setContent(TxToStringDomain(hashBlock, tx));
        return true;
    }
    ...
    if (Address.IsValid()) {
        std::string Content = AddressToStringDomain(Address);
        if (Content.empty())
            return false;
        setContent(Content);
        return true;
...
void DomainExplorer::goTo(const QString& query)
{
    if (switchTo(query)) {
...
void DomainExplorer::onSearch()
{
    goTo(ui->searchBox->text());
}

void DomainExplorer::setBlock(CBlockIndex* pBlock)
{
    setContent(BlockToStringDomain(pBlock));
}

void DomainExplorer::setContent(const std::string& Content)
{
...
void DomainExplorer::updateNavButtons()
{
