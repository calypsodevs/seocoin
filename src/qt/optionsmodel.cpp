...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SEO);
    ...
    if (!settings.contains("nAnonymizeSeoCoinAmount"))
        settings.setValue("nAnonymizeSeoCoinAmount", 1000);

    nAnonymizeSeoCoinAmount = settings.value("nAnonymizeSeoCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSeoCoinAmount"))
        SoftSetArg("-anonymizeseocoinamount", settings.value("nAnonymizeSeoCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in seocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSeoCoinAmount:
            return QVariant(nAnonymizeSeoCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSeoCoinAmount:
            nAnonymizeSeoCoinAmount = value.toInt();
            settings.setValue("nAnonymizeSeoCoinAmount", nAnonymizeSeoCoinAmount);
            emit anonymizeSeoCoinAmountChanged(nAnonymizeSeoCoinAmount);
            break;
        case CoinControlFeatures:
