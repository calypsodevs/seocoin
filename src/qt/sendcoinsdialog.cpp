...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    QString domainrecord = "";//entry->addAsLabel->value() + ui->editDomain->value() + ":" + ui->domainTraffic->value();
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        ...
        send(recipients, strFee, formatted, domainrecord);
        return;
    }
...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted, QString domainRecord)
{
    // prepare transaction for getting txFee earlier
    WalletModelTransaction currentTransaction(recipients);


//add op_return domain record
