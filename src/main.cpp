...
#if defined(NDEBUG)
#error "SEOCOIN cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zSEO is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSEO spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSEO without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSEO spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zSEO spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSEO spend tx %s already in block %d",
            ...
            // Check that zSEO mints are not already known
...
static std::string ScriptToString_(const CScript& Script, bool Long = false, bool Highlight = false)
{
    if (Script.empty())
        return "unknown";
...
std::string SearchTransaction(std::string search)
{
...
//LogPrintf("ReadBlockFromDisk %d\n", pindex);
		...
		for (unsigned int i = 0; i < tx.vout.size(); i++) 
		{
		    ...
		    std::string trout;
		    trout.append(ScriptToString_(txout.scriptPubKey));
//LogPrintf("ScriptToString(txout.criptPubKey) %s\n", ScriptToString_(txout.scriptPubKey));
		    if (trout.find(search) != std::string::npos) 
		    {
    			results.append(trout).append("\n");
		    }
		}
            }    
        }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSEOStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zSEO is staked, masternode only gets 2 SEO
        ret = 3 * COIN;
        if (isZSEOStake)
            ret = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SEOCOIN
...
{
    RenameThread("seocoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSEOMinted()
{
...
void RecalculateZSEOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSEO supply
...
bool RecalculateSEOSupply(int nHeightStart)
{
...
bool UpdateZSEOSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSEOSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSEO money supply in the block index
    ///////if (!UpdateZSEOSupply(block, pindex))
        ///////return state.DoS(100, error("%s: Failed to calculate new zSEO supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SEO to zSEO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSEO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSEO serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSEO stake block is less than allowed start height", __func__);

    if (CZClpStake* zSEO = dynamic_cast<CZClpStake*>(stake)) {
        CBlockIndex* pindexFrom = zSEO->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSEO stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSEO stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSEO->GetValue()));
        if (nChecksum200 != zSEO->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSEO->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSEO() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSEO fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSEO mints and %d zSEO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SEOCOIN: We use certain sporks during IBD, so check to see if they are
