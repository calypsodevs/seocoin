...
int nAnonymizeSeoCoinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "seocoin" is a composite category enabling all SEOCOIN-related debug output
            if (ptrCategory->count(string("seocoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SEOCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SEOCOIN
// Mac: ~/Library/Application Support/SEOCOIN
// Unix: ~/.seocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SEOCOIN";
#else
    fs::path pathRet;
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty seocoin.conf if it does not exist
