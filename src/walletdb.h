...
public:
    ...
    bool WriteZSEOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSEOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSEOSeed_deprecated(uint256& seed);
    bool EraseZSEOSeed();
    bool EraseZSEOSeed_deprecated();

    bool WriteZSEOCount(const uint32_t& nCount);
    bool ReadZSEOCount(uint32_t& nCount);
