...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SEO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSEO send
enum ZerocoinSpendStatus {
    ZSEO_SPEND_OKAY = 0,                            // No error
    ZSEO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSEO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSEO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSEO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSEO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSEO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSEO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSEO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSEO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSEO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSEO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSEO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSEO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSEO_SPENT_USED_ZSEO = 14,                      // Coin has already been spend
    ZSEO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSEO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSEOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzSEOWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSEOTracker> zclpTracker;
    ...
    void setZWallet(CzSEOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zclpTracker = std::unique_ptr<CzSEOTracker>(new CzSEOTracker(strWalletFile));
    }

    CzSEOWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSEO reset */
    boost::signals2::signal<void()> NotifyzSEOReset;
