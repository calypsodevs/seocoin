...
#ifndef SEOCOIN_ZSEOWALLET_H
#define SEOCOIN_ZSEOWALLET_H
...
public:
    CzSEOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSEO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSEO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SEOCOIN_ZSEOWALLET_H
