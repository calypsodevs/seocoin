// Copyright (c) 2018 The SEOCOIN developers
...
CzSEOTracker::CzSEOTracker(std::string strWalletFile)
{
...
CzSEOTracker::~CzSEOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSEOTracker::Init()
{
...
bool CzSEOTracker::Archive(CMintMeta& meta)
{
...
bool CzSEOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSEOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSEOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSEOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSEOTracker::GetSerialHashes()
{
...
CAmount CzSEOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSEOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSEOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSEOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSEOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSEOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSEOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSEOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSEOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSEOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSEOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSEOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSEOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSEOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSEOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSEOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSEOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
