...
#include "config/seocoin-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "seocoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "seocoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SEO and zSEO money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SEO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SEO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, seocoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SEO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SEOCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-clpstake=<n>", strprintf(_("Enable or disable staking functionality for SEO inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zclpstake=<n>", strprintf(_("Enable or disable staking functionality for zSEO inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzclp=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSEO minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zclpbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSEO backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeseocoinamount=<n>     " + strprintf(_("Keep N SEO anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SEOCOIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SEOCOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SEOCOIN Core is shutting down."));
...
#endif
    // Make sure only a single SEOCOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SEOCOIN Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //SEOCOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SEOCOIN specific: zerocoin and spork DB's
                ...
                // SEOCOIN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZSEOMinted();
                        RecalculateZSEOSpent();
                    }
                    RecalculateSEOSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // SEOCOIN: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SEOCOIN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SEOCOIN Core to complete") << "\n";
        ...
        zwalletMain = new CzSEOWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSEOWallet
        uiInterface.InitMessage(_("Syncing zSEO wallet..."));
	...
	boost::filesystem::path pathDomain = GetDataDir() / "seoindex.dat";

	//load when block sync first time
	if (!boost::filesystem::exists(pathDomain))
	{
		FILE* fileout = NULL;
		fileout = fopen(pathDomain.string().c_str(), "a");
		if (fileout) setbuf(fileout, NULL); // unbuffered

		LogPrintf("Loading domains\n");

		int tHeight = chainActive.Height();
		CBlockIndex* pindexSlow = NULL;
		std::string results = "Offer\tdomain name\tvisitors daily\tcontact info\tprice\n";

		for(int pindex=0; pindex < tHeight; pindex++)
		{
			pindexSlow = chainActive[pindex];

			CBlock block;
			if (ReadBlockFromDisk(block, pindexSlow))
			{
				if(pindex % 10000 == 0) {LogPrintf("Search block %d\n", pindex);}
			
		                std::string tr_list;

				BOOST_FOREACH (const CTransaction& tx, block.vtx)
	            		{
                			std::string tr_text = EncodeHexTx(tx);
                        	        //LogPrintf("tx %s\n", tr_text);

					int len = tr_text.length();
					std::string newString;
					for(int i=0; i< len; i+=2)
					{
						string byte = tr_text.substr(i,2);
						char chr = (char) (int)strtol(byte.c_str(), NULL, 16);
						newString.push_back(chr);
					}

					if(newString.find("<data>")!=std::string::npos)
					{
					    std::string record = newString.substr(newString.find("<data>") + 6);
					    std::string record_dm = record.substr(0, record.find("</data>"));
						LogPrintf("ascii %s\n", record_dm);
						//out << record_dm;	
						fprintf(fileout, "%s ", record_dm.c_str());
                                                fprintf(fileout, "%s ", "\n");
...
//    }
//
//    nAnonymizeSeoCoinAmount = GetArg("-anonymizeseocoinamount", 0);
//    if (nAnonymizeSeoCoinAmount > 999999) nAnonymizeSeoCoinAmount = 999999;
//    if (nAnonymizeSeoCoinAmount < 2) nAnonymizeSeoCoinAmount = 2;
    ...
    LogPrintf("Anonymize SEOCOIN Amount %d\n", nAnonymizeSeoCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1SEO+1000 == (.1SEO+100)*10
       10SEO+10000 == (1SEO+1000)*10
    */
